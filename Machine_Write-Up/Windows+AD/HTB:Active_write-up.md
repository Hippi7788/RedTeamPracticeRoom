...
### 主要發現：
1. 漏洞 ：透過SMB共享配置錯誤獲得敏感訊息（風險等級：High）
2. 漏洞 ：透過已知GPP安全性缺陷取得群組原則中的明文密碼（風險等級：Critical）

### 攻擊鏈摘要：

攻擊者利用SMB共享資料夾的配置錯誤取得敏感的群組原則偏好設定 (GPP) 檔案，隨後透過解密與Kerberoasting攻擊手段，最終成功獲取網域管理員權限並掌握系統完全控制權。

### 潛在影響：

1. SMB敏感資料夾過度開放，導致資訊洩露。
2. GPP已知安全性缺陷，允許攻擊者取得群組原則中的明文密碼。
3. 服務帳號密碼強度不足，易受暴力破解，導致憑證洩漏。
4. 特權帳號（Administrator）不應具備SPN，易造成管理員憑證洩漏，導致網域全面崩潰。

### 修復建議：

1. 移除所有Groups.xml等組態檔中的cpassword屬性，並安裝微軟發布的MS14-025補丁。
2. 限制對SMB資料夾的存取權限，僅允許授權使用者讀取。
3. 具備SPN的帳號設定25字元以上的複雜密碼。
4. 解除特權帳號（Administrator）的SPN設定。
...
1. 攻擊鏈清晰，沒有太多橫向移動或內部枚舉的彎彎繞繞，推薦給新手建立AD的進攻模型。
2. 可以練習SMB枚舉，可嘗試使用多種枚舉工具，對比看看效果。
3. 可熟悉常用impacket工具，是AD滲透裡很重要的練習對象，新手不要急著拿Flag，要多試其他工具。
4. 時間同步必須養成習慣。
5. 有多種工具可使用時，務必列出篩選原因。

...
### 主要發現：
1. 漏洞 ：透過不安全的帳戶預驗證設定獲得系統控制權（風險等級：High）
2. 漏洞 ：透過註冊表殘留敏感憑證達成橫向移動（風險等級：High）
3. 漏洞 ：利用服務帳戶被授予的過高權限獲得管理員憑證（風險等級：High）

### 攻擊鏈摘要：

從公開資訊偵察開始，透過網頁員工名單生成帳戶字典後，利用AS-REP Roasting取得初步存取權限。進入系統後，經由本地枚舉從Windows註冊表中提取了高權限帳戶的明文憑證進行橫向移動。最終，利用該帳戶對網域具備的DCSync權限，直接向網域控制站請求管理員雜湊值，成功接管整個網域。

### 潛在影響：

1. 不安全的帳戶預驗證設定導致使用者憑證外洩。
2. 註冊表殘留敏感憑證導致高權限等級使用者憑證外洩，造成網域重要資料外洩。
3. 服務帳號權限過高，導致攻擊者可發起DCSync攻擊，導致管理員憑證外洩，造成網域全面崩潰。
4. 攻擊者可配置持久化手段，有長期資訊洩漏的風險。

### 修復建議：

1. 檢查所有使用者帳戶，確保不要要求Kerberos預身份驗證選項未被勾選。
2. 禁用Windows自動登入功能，並清除註冊表下的敏感鍵值。
3. 審核網域徑的ACL設定，移除不必要使用者相關權限，僅允許合法的網域控制站執行網域同步操作。
4. 部署針對ID 4662(物件存取)的審核策略，監控非domain controller帳戶觸發的複寫請求，以及早偵測DCSync攻擊。

...
### 主要發現：
1. 漏洞 ：權限配置不當的SMB共享資料夾導致機密檔案洩漏（風險等級：Critical）
2. 漏洞 ：透過設置了弱密碼的WinRM備份zip檔和其中的PFX檔獲得遠端登入金鑰和證書（風險等級：Critical）
3. 漏洞 ：透過未被妥善清理的歷史資料洩漏高權限使用者的明文憑證（風險等級：High）

### 攻擊鏈摘要：

攻擊者被允許匿名登入SMB協議，Shares資料夾可讀，洩漏關於WinRM的備份zip檔，此zip檔和其中的PFX檔均使用弱密碼，破解後可透過openssl獲得金鑰和證書，攻擊者可獲得系統控制權。未被妥善清理的歷史資料中存有明文憑證，攻擊者可橫向移動至一位屬於LASP_Readers組的使用者，這個使用者可閱讀ms-Mcs-AdmPwd的明文密碼，登入管理員，獲得網域控制權。

### 潛在影響：

1. 權限配置不當的SMB共享資料夾易導致機密檔案外洩。
2. 關於遠端登入的機密檔案使用弱密碼，易被字典攻擊破解，導致遠端登入權限外洩。
3. 歷史資料未被妥善清理，易造成憑證外洩。
4. 攻擊者可利用已掌握的權限植入後門，建立長期隱蔽的存取通道，製造持久化威脅。

### 修復建議：

1. 禁用SMB匿名登入，並落實最小權限原則，僅允許授權使用者存取共享目錄。
2. 提升壓縮檔與憑證的密碼複雜度，避免使用可被輕易破解的弱密碼。
3. 限制開發環境中明文憑證的留存，建議使用憑證管理器或環境變數加密，並定期清理歷史資料等終端紀錄。
4. 嚴格審核AD中擁有ms-Mcs-AdmPwd讀取權限的群組，確保僅有必要的維運帳號具備此權限，而非普通使用者組。

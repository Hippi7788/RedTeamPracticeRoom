...
### 主要發現：
1. 漏洞 ：透過權限配置不當的RPC協議獲得內部使用者資訊（風險等級：High）
2. 漏洞 ：利用設置弱密碼的使用者帳號獲得SMB內部含有明文密碼的敏感文件（風險等級：Critical）
3. 漏洞 ：利用權限設置不當的ADSync服務帳號解密管理員憑證（風險等級：High）

### 攻擊鏈摘要：

首先透過RPC協議成功枚舉內部使用者帳號，隨即針對SMB協議進行密碼噴灑攻擊，利用獲得的弱口令成功登入。接著在SMB共享區發現敏感檔案azure.xml 並取得有效憑證，橫向移動至具備Azure Admins權限的主機。最後，利用Azure AD Connect的同步機制弱點，從資料庫中提取加密密鑰並解密，獲取管理員憑證，取得完全控制權。


### 潛在影響：

1. RPC協議權限配置不當易造成內部資料外洩，增加員工被社交工程攻擊的風險
2. SMB共享資料夾內存有明文密碼，造成系統控制權被接管
3. ADSync服務權限配置不當，造成管理員權限外洩，導致網域被全面接管
4. 管理員憑證外洩，使外部攻擊者可布置持久化手段，造成長期洩漏風險


### 修復建議：

1. 限制RPC匿名訪問，並強制執行複雜密碼策略，啟用多因素驗證 (MFA)
2. 定期掃描SMB共享區，禁止將包含憑證的XML、腳本或設定檔以純文字形式存放
3. 嚴格限制Azure Admins群組成員，並針對Azure AD Connect所在的伺服器實施最高等級的安全防護
4. 更新Azure AD Connect至最新版本，並監控是否有異常存取加密金鑰（如SQL本地存取）的行為

...
### 主要發現：
1. 漏洞 ：透過公開的/.git目錄獲得明文憑這和版本訊息（風險等級：High）
2. 漏洞 ：透過已知版本檔案上傳漏洞上傳惡意tar檔執行代碼並獲得Webshell（風險等級：High）
3. 漏洞 ：透過重複使用的密碼獲得系統使用者的控制權（風險等級：Medium）
4. 漏洞 ：透過無須密碼的sudo權限達成權限提升（風險等級：High）

### 攻擊鏈摘要：

攻擊者首先透過目錄爆破發現並下載/.git資料夾，從中提取出明文憑證與軟體版本資訊。接著利用該憑證登入系統，藉由軟體上傳漏洞植入Webshell並建立反彈Shell。隨後，攻擊者利用重複使用的密碼嘗試SSH登入成功，最終利用sudo bee指令的配置錯誤成功提升權限至root，達成目標主機的完全控制。

### 潛在影響：

1. 完全失去控制權： 攻擊者獲得root最高權限，可任意讀取、修改或刪除系統內所有資料。
2. 機敏資料外洩： 包括原始碼、資料庫帳密及使用者私密資料皆已被看穿。
3. 橫向移動風險： 攻擊者可以此主機為跳板，進一步攻擊內部網路的其他設備。

### 修復建議：

1. 環境加固： 在Web Server設定檔中禁用.git等隱藏目錄的存取權限。
2. 漏洞修補： 立即將存在上傳漏洞的軟體更新至安全版本，或更換安全性更高的替代方案。
3. 憑證管理： 嚴禁在原始碼中寫入明文密碼；實施密碼隔離，系統SSH密碼應與應用程式帳號完全不同。
4. 最小權限原則： 重新審核/etc/sudoers配置，移除不必要的NOPASSWD權限。若必須使用bee指令，應限制其參數或使用更安全的呼叫方式。

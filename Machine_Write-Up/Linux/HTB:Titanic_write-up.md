...
### 主要發現：
1. 漏洞 ：應用程式任意檔案讀取漏洞（風險等級：High）
2. 漏洞 ：Gitea資料庫憑證洩漏與弱密碼雜湊（風險等級：High）
3. 漏洞 ：自動化腳本內舊版ImageMagick本地提權漏洞（風險等級：Critical）

### 攻擊鏈摘要：

攻擊者透過ffuf進行子域名模糊測試，成功發現dev.開發子域名，訪問該子域名發現Gitea服務，並在/developer路徑下發現兩個專案。從docker-config專案中取得路徑與資料庫憑證；從flask-app專案原始碼中發現對使用者輸入的檔名未進行過濾。利用上述未過濾的檔案輸入點，使用curl驗證任意檔案讀取漏洞，成功讀取/etc/passwd並確認系統內僅有root與developer具備/bin/bash權限。參考Gitea官方文檔定位設定檔app.ini並找出SQLite資料庫路徑，枚舉user資料表，成功解密developer使用者的密碼雜湊。透過SSH以developer帳號成功登入系統，在/opt/scripts中發現一個以自動排程執行的.sh腳本，該腳本呼叫的ImageMagick版本存在公開的已知漏洞，利用此漏洞成功提權至root權限。

### 潛在影響：

1. 完全控制系統：攻擊者可取得作業系統最高權限，任意修改系統設定、植入後門或清除日誌。
2. 機敏資料外洩：原始碼、資料庫憑證及使用者密碼完全暴露，可能導致其他關聯系統同步遭殃。
3. 開發環境污染：攻擊者可竄改Gitea內的程式碼，發動供應鏈攻擊。

### 修復建議：

1. 輸入驗證與過濾：對應用程式中所有涉及檔案讀取的輸入進行嚴格限制（如：實作檔名白名單、禁止 ../ 等路徑穿越字元），或改用安全的安全API讀取檔案。
2. 權限與憑證隔離：禁止將生產環境或含敏感憑證的設定檔上傳至權限控管不當的公開/開發庫中，Gitea的資料庫檔案應嚴格限制作業系統讀取權限。
3. 密碼策略強化：提升密碼複雜度要求，並確保密碼雜湊演算法使用強固機制，增加離線破解難度。
4. 軟體更新與排程審查：立即將ImageMagick升級至安全性修正版本。審查/opt/scripts中自動排程的執行權限，遵循最小權限原則，避免以高權限執行不必要的第三方工具。
